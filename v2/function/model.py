	...
	def forward(self, _input, Xavg, Xstd, name, trainAdv):
		...
		if 'UnetAE' in name:
			...
			if not trainAdv and 'prePP' in name:
				l2 = self.pitch_decode(vec.detach()) # predict pitch adversarial update D
			if trainAdv and 'prePN' in name:
				l4 = self.pitch_decode(vec) # predict pitch adversarial update E 
			if not trainAdv and 'preRoll' in name:
				l5 = self.roll_decode(vec,name,s,c) # predict pianoroll
			vec = vec
		...
		if 'DuoAE' in name:
			...
			if trainAdv and 'preIN' in name:
				l3 = self.inst_decode(vec_p) # predict instrument adversarial 
			if trainAdv and 'prePN' in name:
				l4 = self.pitch_decode(vec_i) # predict pitch adversarial
			if not trainAdv and 'preRoll' in name:
				l5 = self.roll_decode(torch.cat((vec_i,vec_p),1),name,s) # predict pianoroll
			vec = vec_i
